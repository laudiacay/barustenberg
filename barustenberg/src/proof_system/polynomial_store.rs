use std::{collections::HashMap, fmt::{Formatter, self, Display}};
...
impl<Fr:Field> PolynomialStore<Fr> {
    ...
    const fn get (&self, key: String) -> Result<Polynomial<Fr>> {
        self.polynomial_map.get(&key).ok_or(anyhow!("didn't find polynomial...")).cloned()
    }
    ...
    const fn remove(&mut self, key: String) -> Result<Polynomial<Fr>> {
        self.polynomial_map.remove(&key).ok_or(anyhow!("didn't find polynomial..."))
    }
    ...
    fn get_size_in_bytes(&self) -> usize {
        ...
        for (_, entry) in self.polynomial_map.iter() {
            size_in_bytes += entry.size() * Fr.SizeInBytes;
        };
        size_in_bytes
    }
...
impl<Fr: Field> Display for PolynomialStore<Fr> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        for (key, entry) in self.polynomial_map.iter() {
            let entry_bytes = entry.size() * Fr.SIZE_IN_BYTES;
            write!(f, "PolynomialStore: {} -> {} bytes, {:?}", key, entry_bytes, entry);
        }
        Ok(())
    }
}
